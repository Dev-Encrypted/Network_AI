...
> **Current software: v0.14 private application and source tooling, alongside the preserved F0 research bench.** Model offers can now bind an explicit generation policy and exact chat template across the catalog, signed authorization and root runtime. The coordinator, gateway, interface, agents, links and model launcher have owned Windows process trees and recoverable launch identities. An official 32B model runs across a budgeted NVIDIA CUDA contributor and a CPU contributor on one computer, with shared-resource admission, signed receipts and tested crash recovery. Public decentralization, independent operators, broader GPU/host qualification, approved issuance and the paid marketplace remain unfinished. Start with [generation profiles](docs/implementation/GENERATION_PROFILES.md), the [device guide](docs/implementation/HETEROGENEOUS_CONTRIBUTORS.md), and [measured implementation status](docs/implementation/STATUS.md).

The original fixed functional campaign accepted **10 of 16 samples** across the CPU and CPU/CUDA routes. All 16 requests completed and settled, but both routes failed three of the eight answer requirements. This distinction matters: correct accounting does not certify a correct answer. The [qualification guide](docs/implementation/MODEL_QUALIFICATION.md) includes the procedure, preserved failures, actual context limits and evaluator corrections.

A separate v0.14 CPU comparison accepted **5/8 cases with the default template and 6/8 with thinking explicitly disabled**. Two cases improved and one regressed; both complete campaigns still fail. The [generation-policy results](docs/implementation/GENERATION_PROFILES.md#measured-32b-comparison) include every case, unchanged token allowances, actual charges and the restored original environment.
...
| B: a nearby GPU cluster | An operator offers a model served by a tightly connected group of GPUs | Larger models requiring several nearby GPUs | CPU and mixed CPU/CUDA routes executed on one host; broader multi-GPU qualification remains |
| C: model split across participants | Different nodes execute successive parts of the same model | Models requiring capacity from several participants | Real 32B CPU and mixed CPU/CUDA routes with signed stage agents on one host; independent-provider qualification remains |
...
Version 0.6 connects **existing-credit funding, accepted readiness and consumption recycling**. Working capital and protected reserve are separate; consumption replenishes the working floor, reserve and working target in order. A live 32B request returned 42,000 microcredits to its fund, which paid for a second 30-second route window without another contribution or grant. Operators accepted readiness-only compensation for those pool sessions; ordinary inference retains its previous terms. This validates one-account transaction behavior, not independent economic sustainability. [Beginner explanation, formulas, API and evidence](docs/implementation/COOPERATIVE_FUNDS.md).


Version 0.9 adds [guarded model-stage traffic over QUIC](docs/implementation/GUARDED_RPC_TRANSPORT.md): paired transport identities bind each connection to one stage, route and model manifest before opening the local guard. Real 32B inference, concurrent-call serialization, a zero-charge link failure and group recovery are tested on this computer. This is a private transport implementation; separate physical hosts, broader GPU compatibility and public operator trust still need qualification.

Version 0.10 adds [portable contributor supervision](docs/implementation/PORTABLE_CONTRIBUTORS.md). Each contributor runs its own pinned CPU worker and both transport peers, while short-lived signed readiness declarations remove the need for the root backend key. Strict profiles reject unrelated credentials, child environments are restricted, and component failure drains the contributor. The [standalone package manual](packages/contributor/README.md) explains prerequisites, peer exchange, commands, source packaging and recovery. The one-computer evidence does not establish independent providers or an OS sandbox.

Version 0.11 adds [Windows contributor crash containment](docs/implementation/CONTRIBUTOR_PROCESS_CONTAINMENT.md). A native guardian establishes an anonymous kill-on-close job before the CPU worker and transports start, so a terminated supervisor does not leave those descendants running. The stopped-profile upgrade preserves keys, transport pins and model state. OS-user isolation and hostile-peer resource limits remain separate work; v0.13 extends lifecycle ownership to the root and coordinator services.

Version 0.12 adds [explicit CPU/CUDA contribution profiles](docs/implementation/HETEROGENEOUS_CONTRIBUTORS.md). The measured 32B route uses 1:9 layer weights, a 3 GiB CUDA buffer budget and a 20 GiB CPU buffer budget. Its requests acquire both existing resource domains, so the original GPU model and CPU route cannot sell those same slots simultaneously. A CUDA worker failure refunded the request and released both claims; reload restored inference with the same identities. These are observations on one RTX 4090 and one CPU, not general hardware qualification or a calibrated exchange price. [Exact campaign and limitations](docs/implementation/evidence/qwen3-32b-mixed-devices-v0.12.json).

Version 0.13 adds [managed Windows service supervision](docs/implementation/SERVICE_SUPERVISION.md). Separate supervisors own the coordinator, gateway, interface, links, agents and root-model launcher. Durable launch identities prevent duplicate launches after an interrupted command, and a kernel mutex serializes registry updates. The actual 32B root-host fault campaign checks descendant cleanup, refund, sibling preservation and inference after full route reload. Installed-service startup, reboot recovery and public availability guarantees remain unfinished.

Version 0.7 adds bounded automatic renewal with independent manager spending authority and provider participation limits. Version 0.8 adds **demand-backed expansion**: an extra complete-route window can use only working credits above the essential floor, with a full protected reserve, current support and separately classified funded consumer pressure. Self-use, account aliases, grants and the same waiting request cannot manufacture repeated expansion. Existing contracts remain protected when future permissions are revoked. Positive activation is tested in isolated fixtures; actual one-computer evidence records why growth stayed blocked. [Step-by-step explanation, arithmetic, API and evidence](docs/implementation/DEMAND_BACKED_EXPANSION.md).
...
| Guarded model-data QUIC | Installed 32B weights and execution pass through two authenticated, route-bound pairs; receipts, zero-charge failure and reloaded recovery checked | One physical host and CPU domain; relay disabled; no independent-host or WAN qualification |
