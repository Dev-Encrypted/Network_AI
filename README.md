...
> **Current software: v0.12 private device preview, alongside the preserved F0 research bench.** An official 32B model now runs across a budgeted NVIDIA CUDA contributor and a CPU contributor on one Windows computer. Device selection, unequal layer placement, guarded allocation budgets, shared-resource admission, signed receipts and crash recovery are implemented. The source package retains participant-owned supervision and Windows process containment. Public decentralization, independent operators, broader GPU/host qualification, approved issuance and the paid marketplace remain unfinished. Start with the [device and memory guide](docs/implementation/HETEROGENEOUS_CONTRIBUTORS.md), [contributor package](packages/contributor/README.md), and [measured implementation status](docs/implementation/STATUS.md).
