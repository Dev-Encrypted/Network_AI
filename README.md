...
> **Current software: v0.7 private bounded-renewal preview, alongside the preserved F0 research bench.** The application includes real inference, signed nodes, authenticated private QUIC links, complete routes and cooperative funds. An official 32B model runs through two guarded CPU stages on one computer. Consumed credits can finance subsequent readiness windows automatically, within separate manager spending limits and provider participation mandates. Revocation stops future windows while preserving accepted obligations. Public decentralization, approved issuance, independent multi-host qualification and the paid marketplace remain under development. Read the [renewal guide](docs/implementation/BOUNDED_RENEWALS.md) and [implementation matrix](docs/implementation/STATUS.md).
