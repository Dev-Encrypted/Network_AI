...
> **Current software: v0.11 private contributor preview, alongside the preserved F0 research bench.** Contributors supervise their own CPU worker, guard and authenticated control/model-data links without root model API or coordinator credentials. A standalone source package provides identity, configuration, binary verification, start, status, stop and upgrade commands. A pinned Windows guardian now contains each contributor's normal process tree when its supervisor or guardian crashes. The official 32B route retains exact route/model bindings, one-slot admission, all-participant receipts and cooperative credits on one computer. Public decentralization, approved issuance, independent multi-host/GPU qualification and the paid marketplace remain under development. Read the [contributor guide](docs/implementation/PORTABLE_CONTRIBUTORS.md), [Windows containment and upgrade guide](docs/implementation/CONTRIBUTOR_PROCESS_CONTAINMENT.md), and [implementation matrix](docs/implementation/STATUS.md).
