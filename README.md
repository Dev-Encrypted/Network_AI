...
> **Current software: v0.10.1 private contributor preview, alongside the preserved F0 research bench.** Contributors can supervise their own CPU worker, guard and authenticated control/model-data links without a root model API key or coordinator credentials. A standalone source package provides identity, configuration, binary verification, start, status and stop commands. The official 32B route uses two contributor profiles on one computer, retaining exact route/model bindings, one-slot admission, all-participant receipts and cooperative credits. The latest patch corrects a Windows status-file failure, verified during real 32B execution. Public decentralization, approved issuance, independent multi-host/GPU qualification and the paid marketplace remain under development. Read the [contributor guide](docs/implementation/PORTABLE_CONTRIBUTORS.md), [patch evidence and upgrade instructions](docs/implementation/CONTRIBUTOR_STATUS_RECOVERY.md), and [implementation matrix](docs/implementation/STATUS.md).
