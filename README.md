...
> **Current software: v0.8 private demand-backed expansion preview, alongside the preserved F0 research bench.** The application includes real inference, signed nodes, authenticated private QUIC links, complete routes and cooperative funds. An official 32B model runs through two guarded CPU stages on one computer. Consumed credits can finance subsequent readiness windows within finite manager and provider permissions. Additional windows require funded demand, protected essential funds, private party/support evidence and another complete free route. The real one-host campaign correctly refused expansion while continuing useful inference. Public decentralization, approved issuance, independent multi-host qualification and the paid marketplace remain under development. Read the [expansion guide](docs/implementation/DEMAND_BACKED_EXPANSION.md) and [implementation matrix](docs/implementation/STATUS.md).
