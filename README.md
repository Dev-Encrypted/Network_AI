...
> **Current software: v0.6 private cooperative preview, alongside the preserved F0 research bench.** The application includes real inference, signed nodes, authenticated private QUIC links, complete routes and funded readiness. An official 32B model runs through two guarded CPU stages on one computer. Opted-in consumption can now replenish a community fund and finance another whole-route window, with explicit readiness-only provider consent, protected reserve rules and exactly backed refunds. Public decentralization, approved issuance, independent multi-host qualification and the paid marketplace remain under development. See the [implementation matrix](docs/implementation/STATUS.md).
