...
> **Current software: v0.9 private guarded RPC transport preview, alongside the preserved F0 research bench.** The application includes real inference, signed nodes, complete routes and cooperative credits. The official 32B model now loads and executes through two guarded CPU stages over authenticated QUIC on one computer. Exact route/model bindings, one-slot admission and all-participant receipts remain enforced. Bounded renewal and demand-backed expansion remain available under their accepted private rules. Public decentralization, approved issuance, independent multi-host qualification and the paid marketplace remain under development. Read the [model-data transport guide](docs/implementation/GUARDED_RPC_TRANSPORT.md) and [implementation matrix](docs/implementation/STATUS.md).
