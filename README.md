...
> **Current software: v0.13.1 source tooling with the v0.13 private application, alongside the preserved F0 research bench.** The coordinator, gateway, interface, node agents, links and root-model launcher have owned Windows process trees and recoverable launch identities. The patch handles cancellation during startup and adds functional model qualification. An official 32B model runs across a budgeted NVIDIA CUDA contributor and a CPU contributor on one computer, with shared-resource admission, signed receipts and tested crash recovery. Public decentralization, independent operators, broader GPU/host qualification, approved issuance and the paid marketplace remain unfinished. Start with [service supervision](docs/implementation/SERVICE_SUPERVISION.md), the [device guide](docs/implementation/HETEROGENEOUS_CONTRIBUTORS.md), and [measured implementation status](docs/implementation/STATUS.md).

The latest fixed functional campaign accepted **10 of 16 samples** across the CPU and CPU/CUDA routes. All 16 requests completed and settled, but both routes failed three of the eight answer requirements. This distinction matters: correct accounting does not certify a correct answer. The [qualification guide](docs/implementation/MODEL_QUALIFICATION.md) includes the procedure, preserved failures, actual context limits and evaluator corrections.
