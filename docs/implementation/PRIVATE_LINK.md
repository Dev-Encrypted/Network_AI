...
**Relay service, CGNAT traversal, WAN performance, independent hosts and public admission are not qualified.** Relays are disabled in this version. This link also does not carry model-stage tensors, provide Byzantine consensus, conceal prompts from the GPU operator, or establish a complete distributed model route.
