...
Each command first prepares its OS query runtime once, with a separate limit of twenty seconds, before acquiring a registry mutex or reserving a launch. This reads the query process itself and requires an exact readiness acknowledgement; a failed preparation fails startup. It does not reuse that probe as another process's identity. Every later identity query keeps its five-second limit. A new persistent service host establishes its native job before preparing its own query runtime, so its temporary PowerShell child is also contained. This separates measured cold OS initialization from normal identity queries and application/model readiness.
