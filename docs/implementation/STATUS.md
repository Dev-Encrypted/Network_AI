...
## Version 0.11 contributor containment

A pinned native Windows guardian now owns each contributor's anonymous kill-on-close job. It confirms its actual parent, current boot, process membership and non-inheritable handle before the supervisor starts any worker or transport. Supervisor or guardian termination ends the contributor's normal process tree. The upgrade command preserves existing identities, private signing state, model/route bindings and transport pins. Missing or failed protection refuses startup. This is lifecycle containment; OS access isolation, resource limits and root/coordinator service ownership remain separate work. [Mechanism and upgrade instructions](CONTRIBUTOR_PROCESS_CONTAINMENT.md).

The actual one-computer 32B campaign completed **six requests**, each with 18 input/six output tokens, three accepted receipts and a conserved 36,000-micro-LAB_TU charge. Concurrent consumers serialized on the single physical slot. Supervisor, guardian and worker failures during inference each reached `FAILED / REFUNDED`, charged zero and released every physical claim. All five affected contributor processes and their TCP/UDP listeners were gone; the sibling contributor stayed alive. Whole-route reload restored real inference after each fault. Terminal/refunded observation took 35,269 ms, 35,452 ms and 1,543 ms respectively; these are not OS kill latencies or a guaranteed recovery time. [Measured containment campaign](evidence/qwen3-32b-contributor-containment-v0.11.json).

The [status-lock regression](evidence/qwen3-32b-contributor-status-v0.11.json) also passed with the guardian active: the same supervisor and children survived a 9,033 ms Windows read lock while a real 23-input/128-output-token request settled three receipts and 407,000 micro-LAB_TU. Both campaigns used existing credits and had no projection mismatch.

Validation passed **167 distinct automated cases**: 94 integration, 22 Node unit/socket, 16 Rust, 21 Python F0, ten browser journeys and four actual Windows process-tree fixtures. The new Windows CI job covers the disposable lifecycle fixtures without model weights. Real 32B execution remains a local observation on the user's single computer. The [isolated working-database restore](evidence/contributor-backup-restore-v0.11.json) preserved 605 journals, balanced projections and runtime permissions. The [source archive](evidence/contributor-package-v0.11.json) includes the participant source and bundled dependency; native executables, Node, engines, models and private state are separate prerequisites. [Versioned validation and measurement limits](validation-v0.11.json).

The project continues to prioritize models above 27B. This version does not establish independent GPU/host compatibility, a public peer network, successful economic validation, approved issuance, hostile-work verification, distributed continuity or cash settlement. Applied migrations 001–009, earlier tags/packages and preserved F0 sources retain their original bytes. The following sections record their respective historical versions.
