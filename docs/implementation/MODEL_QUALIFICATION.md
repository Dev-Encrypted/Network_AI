...
Version 0.14 adds a separate [generation-policy comparison](GENERATION_PROFILES.md#measured-32b-comparison). New immutable CPU offers, using the same corpus and output allowances, accepted 5/8 cases with the default template and 6/8 with thinking explicitly disabled. A corrected conversation and raw array were accompanied by a structured-extraction formatting regression. Both full campaigns still fail. Those 16 new requests and their charges are recorded separately from the original 16; no answer key, original report or historical denominator is changed.
