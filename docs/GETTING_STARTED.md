...
The [contributor package](../packages/contributor/README.md) lets an invited CPU or explicitly selected CUDA stage run its own worker and private transports. The [device guide](implementation/HETEROGENEOUS_CONTRIBUTORS.md) explains compatible engine packages, memory budgets, unequal placement, and the one-computer evidence boundary. It needs the participant's own identity, accepted route details and pinned binaries, including the v0.11 Windows lifecycle guardian; it does not need the root model API key or database credentials. Start with the package's prerequisite, upgrade and peer-exchange instructions. Two profiles on one machine still count as one physical host.
