...
The [contributor package](../packages/contributor/README.md) lets an invited CPU stage run its own worker and private transports. It needs the participant's own identity, accepted route details and pinned binaries, including the v0.11 Windows lifecycle guardian; it does not need the root model API key or database credentials. Start with the package's prerequisite, upgrade and peer-exchange instructions. Two profiles on one machine still count as one physical host.
