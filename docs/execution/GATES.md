...
Status reflects the preserved F0 evidence and private implementation through v0.14, validated on September 14–15, 2026 (America/Sao_Paulo; raw reports use UTC). CPU/CUDA contributor budgets, unequal placement, signed readiness, contributor containment, managed Windows service ownership, startup cancellation and explicit generation-profile binding are implemented. Actual root-model failure, process cleanup, refund, sibling preservation and reload were measured on one computer. The original fixed functional campaign accepted 10/16 samples. A separate generation-policy comparison accepted 5/8 with the default template and 6/8 with thinking disabled; both still fail full functional acceptance, and the switch regressed one case. Independent-host, broader model/OS/GPU, public-trust and economic gates remain open. A gate requires the stated evidence; a document, passing unit test or public repository is not enough.
...
The recommended product focus is models above 27B. The official 32B private profiles have completed inference through two CPU workers and through a mixed RTX 4090/CPU route. The earlier CPU campaign includes a short parity comparison; the mixed campaign records actual buffers, complete-agent reservations, cross-model resource conflicts, receipts and failure handling. Its [mixed-device evidence](../implementation/HETEROGENEOUS_CONTRIBUTORS.md) does not establish independent operators or broad numerical parity. Next campaigns must broaden quality/context, device classes, physical links, concurrency and recovery. Small controlled baselines remain useful without being promoted to proof of the larger target.
