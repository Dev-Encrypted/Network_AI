...
Status reflects the preserved F0 evidence and private implementation through v0.10, validated on September 14–15, 2026 (America/Sao_Paulo; raw reports use UTC). Portable CPU contributor supervision and signed readiness are implemented; physical-host, GPU, public-trust and economic gates remain open. A gate requires the stated evidence; a document, passing unit test or public repository is not enough.
...
The recommended product focus is models above 27B. The official 32B private profile has now completed inference through two actual CPU workers, with recorded artifact identity, allocations, a short parity comparison, complete-agent reservations, receipts and failure handling. Its [evidence](../implementation/COMPLETE_ROUTES.md) does not establish an independently operated multi-device route. Next campaigns must broaden quality/context and qualify distinct devices, links, concurrency and recovery. Small controlled baselines remain useful without being promoted to proof of the larger target.
