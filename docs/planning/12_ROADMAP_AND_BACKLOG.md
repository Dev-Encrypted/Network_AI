...
| R07: model above 27B | Complete inference on a pinned, licensed larger-model configuration with measured memory and quality | Official 32.8B Q4_K_M executed. Explicit generation/template terms now bind offers, readiness and root capabilities. The original CPU/mixed campaign accepted five cases per route; a separate CPU template comparison accepted 5/8 versus 6/8 with one regression. Full functional acceptance, context and broader quality remain unresolved |
| R08: nearby multi-device route | Supported partition, per-device budget, parity and concurrency evidence | Actual CPU/CUDA 32B route, 1:9 layer placement, observed allocation budgets and shared-domain concurrency demonstrated on one host; broader numerical, device and performance qualification remains |
| R09: distinct physical hosts | LAN and WAN profiles with measured links, NAT/relay behavior and independent failure injection | Portable CPU/CUDA supervision, Windows contributor crash containment, participant-only configuration, signed readiness and both QUIC paths implemented on one host; distinct physical hosts, NAT/relay and hostile-peer qualification remain |
| R10: integrated distributed model | Gateway admission covers every stage; cancellation, receipts and recovery work for the whole route | Real 32B CPU and mixed CPU/CUDA routes, route-bound RPC over QUIC, all-domain reservations, receipts, cross-model exclusion and post-fault reload demonstrated on one host; WAN and adversarial verification remain |
| R11: heterogeneous profiles | Qualified hardware classes with transparent contribution-to-consumption comparisons | Explicit CPU/CUDA selection, immutable execution definitions, independent memory observations and allocator bounds implemented; one RTX 4090 plus CPU measured. Broader classes, useful-capacity benchmarks and fair exchange calibration remain |
