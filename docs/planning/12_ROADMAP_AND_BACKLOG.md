...
| R09: distinct physical hosts | LAN and WAN profiles with measured links, NAT/relay behavior and independent failure injection | Portable worker supervision, Windows contributor crash containment, participant-only configuration, signed readiness and both QUIC paths implemented on one host; physical-host, GPU, NAT/relay and hostile-peer qualification remain |
...
| R18: broader product | Desktop contributor, localization, model adapters, multimodality and locally bounded coding agent | Standalone private CPU contributor CLI/package, explicit legacy upgrade and Windows process-tree guardian available; GUI, root/coordinator service supervision, resource limits, additional hardware/engine profiles and broader product work remain |
