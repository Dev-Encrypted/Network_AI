...
def minimum_core(providers):
    ...
    # Retain every feasible minimum model; a missing giant does not disable compact service.
    core = small[:1]
    if len(large) >= n:
        core += large[:n]
    elif not core:
        core = large[:1]  # A large node can serve compact locally in this fictional profile.
    return core


def simulate(name, seed, reserve_enabled=True, reserve_hours=None, retention_bps=None, recycling_enabled=True):
    spec = SCENARIOS[name]
    ...
    for hour in range(F["days"] * 24):
        ...
        # Keep the target for the physically feasible catalog. A temporary cash-in-TU
        # shortage must not shrink its refill target and burn the funds needed to recover.
        reserve_core_rate = sum(p.base_rate for p in core)
        # Financial degradation is also per model: do not halt compact because a
        # complete giant route costs more than the remaining service budget.
        liquid = tu.balances["continuity"] if reserve_enabled else 0
        candidates = [core] if core else []
        compact_candidates = sorted(online, key=lambda p: (p.base_rate,p.id))
        if compact_candidates:
            candidates.append(compact_candidates[:1])
        affordable = []
        for candidate in candidates:
            new_needed = max(0, sum(p.base_rate for p in candidate)-liquid)
            if new_needed + new_needed // P["continuity_issuance_divisor"] <= room:
                affordable = candidate
                break
        if affordable:
            if affordable != core:
                metrics["hours_with_financial_model_degradation"] += 1
            core = affordable
        core_rate = sum(p.base_rate for p in core)
        target_reserve = reserve_core_rate * reserve_hours
        ...
        liquid = tu.balances["continuity"] if reserve_enabled else 0
        minimum_new = max(0, core_rate-liquid)
        minimum_promise = minimum_new + minimum_new // P["continuity_issuance_divisor"]
        ...
        if not quorum:
            ...
            for provider in core + [p for p in online if p not in core]:
                ...
                liquid = tu.balances["continuity"] if reserve_enabled else 0
                candidate_new = max(0, candidate_reward-liquid)
                candidate_topup = min(max(0, target_reserve-tu.balances["continuity"]+min(candidate_reward,liquid)),
                                      candidate_new // P["continuity_issuance_divisor"])
                if candidate_new + candidate_topup <= room:
                    active.append(provider)
                    selected_reward = candidate_reward
            assert all(p in active for p in core)
            reward_total = selected_reward
            recycled_payment = min(reward_total, tu.balances["continuity"]) if reserve_enabled else 0
            new_reward = reward_total - recycled_payment
            topup = min(max(0, target_reserve-tu.balances["continuity"]+recycled_payment),
                        new_reward // P["continuity_issuance_divisor"])
            # Existing TU can circulate even when a capacity loss puts historical stock above target.
            if new_reward + topup:
                tu.promise(new_reward + topup, ceiling)
            remaining_recycled = recycled_payment
            for provider in active:
                amount = requested_rewards[provider.id]
                from_reserve = min(amount, remaining_recycled)
                remaining_recycled -= from_reserve
                if from_reserve:
                    tu.transfer(("circulation_pay", hour, provider.id), "continuity", provider.id, from_reserve)
                if amount > from_reserve:
                    tu.issue(("ordinary", hour, provider.id), provider.id, amount-from_reserve)
                paid[provider.id] += amount
                metrics["ordinary_rewards_microtu"] += amount-from_reserve
                metrics["circulated_rewards_microtu"] += from_reserve
        ...
        for owner in range(accounts):
            ...
            metrics[("provider" if owner < len(providers) else "non_provider") + "_desired_work"] += work
        ...
        for model in price:
            ...
            for round_index in range(max(F["request_sizes"]) + 1):
                ...
                for offset in range(accounts):
                    ...
                    request = queue[0]
                    # work is a batch of independent unit requests, not a divisible token stream.
                    chunk = min(request.work, remaining, deficits[(model, owner)])
                    cost = chunk * price[model]
                    if not chunk:
                        continue
                    if tu.balances[owner] < cost:
                        metrics["insufficient_tu_observations"] += 1
                        continue
                    before_work = request.work
                    request.work -= chunk
                    if not request.work:
                        queue.popleft()
                    deficits[(model, owner)] -= chunk
                    remaining -= chunk
                    metrics[model + "_executed_work"] += chunk
                    key = ("service", request.id, model, before_work)
                    ...
                    if draw(seed, "failure", request.id, model) < F["failed_service_probability"]:
                        ...
                        metrics["failed_refunded_work"] += chunk
                    else:
                        recycled = min(cost, max(0, target_reserve-tu.balances["continuity"])) if recycling_enabled and reserve_enabled else 0
                        if recycled:
                            tu.transfer((key, "recycle"), "usage_hold", "continuity", recycled)
                            metrics["recycled_consumption_microtu"] += recycled
                        tu.consume((key, "burn"), "usage_hold", cost-recycled)
                        metrics[model + "_served_work"] += chunk
                        metrics[("provider" if owner < len(providers) else "non_provider") + "_served_work"] += chunk
        ...
        if spec.get("fraud") and quorum and state == "NORMAL":
            allowance = metrics["ordinary_rewards_microtu"] * P["experimental_issuance_cap_bps"] // 10000 - metrics["experimental_admission_ceiling_microtu"]
    ...
    return {"scenario":name, "seed":seed, "reserve_enabled":reserve_enabled,
            "reserve_target_hours":reserve_hours, "retention_threshold_bps":retention_bps, "recycling_enabled":recycling_enabled,
...
def boundary_cases():
    ...
    recycle = Ledger({"user":100,"hold":0,"continuity":0})
    recycle.transfer("h","user","hold",100)
    recycle.transfer("r","hold","continuity",60)
    recycle.consume("b","hold",40)
    assert recycle.stock() == 60 and recycle.issued == 0
    cases.append({"id":"C05","scope":"recycled plus burned consumption equals charge without new issuance","status":"passed_reference"})
    recycle.transfer("refund_r","continuity","user",60)
    recycle.refunded += 40
    recycle.balances["user"] += 40
    recycle.audit()
    assert recycle.stock() == recycle.initial == 100
    cases.append({"id":"C06","scope":"reference split refund debits reserve and reverses only burned units","status":"passed_reference"})
    partial = [Provider(0,"small",1),Provider(1,"large",1),Provider(2,"large",1)]
    assert capacities(minimum_core(partial))["large"] == 0
    assert capacities(minimum_core(partial))["compact"] > 0
    assert minimum_core([]) == []
    cases.append({"id":"C07","scope":"missing giant route preserves feasible compact model; no node means no service","status":"passed_reference"})
    return cases


def main():
    runs = [simulate(name, seed) for name in SCENARIOS for seed in F["seeds"]]
    controls = [simulate(name, seed, recycling_enabled=False)
    ...
    for control in controls:
        ...
        comparison.append({"scenario":control["scenario"],"seed":control["seed"],
                           "with_recycling_service_fraction":selected["service_fraction"],
                           "without_recycling_service_fraction":control["service_fraction"],
    ...
    report = {"status":"passed_model_invariants_not_real_world_validation",
              ...
              "summary":summary, "recycling_comparison":comparison,
              "public_activation_gate":{
                  "status":"rejected_fictional_parameter_set",
                  "reason":"Baseline has economic hibernation; passing ledger arithmetic is insufficient.",
                  "baseline_economic_hibernation_hours_max":max(
                      sum(v for k,v in r["state_hours"].items() if k == "HIBERNATING_CREDITS")
                      for r in runs if r["scenario"] == "steady_zero_buyers"),
                  "real_eligible_request_slo_measured":False,
                  "holdout_behavioral_validation_executed":False},
    ...
    print(json.dumps({k:report[k] for k in ("status","total_runs","scenario_count","days_per_run","public_activation_gate","summary","recycling_comparison")},indent=2))
