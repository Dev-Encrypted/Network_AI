...
> Transport update: the [v0.9 model-data path](implementation/GUARDED_RPC_TRANSPORT.md) carries the actual 32B model weights and RPC computation through authenticated, route-bound QUIC peers. Private one-host execution, concurrency, failure/refund and reload recovery are measured. The same physical and economic qualification boundaries remain.
